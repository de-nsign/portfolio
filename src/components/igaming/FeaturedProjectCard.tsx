  ...
  return (
    <Wrapper ref={ref} className={wrapperClassName} {...wrapperProps}>
      ...
      <div className="relative z-[5] flex h-[560px] flex-col px-6 pt-14 text-center">
        ...
        >
          {project.image ? (
            <div className="mx-auto h-full w-[78%] overflow-hidden rounded-t-[20px] border-[3px] border-b-0 border-black bg-black shadow-[0_24px_60px_-20px_rgba(40,20,70,0.35)]">
              <Image
                src={project.image}
                alt={`${project.title} — illustration`}
                width={1920}
                height={1080}
                className={`h-full w-full rounded-t-[17px] object-cover ${imagePos}`}
                priority={priority}
              />
            </div>
          ) : (
            <div className="mx-auto h-full w-[78%] rounded-t-[20px] border-[3px] border-b-0 border-black bg-neutral-200" />
          )}
        </motion.div>
      </div>
